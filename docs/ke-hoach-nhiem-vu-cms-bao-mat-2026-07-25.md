...
**2026-07-30 (lần 13)** — Đưa "Truy Cập Nhanh" lên đầu trang Tổng Quan; URL sản phẩm sạch hơn (`san-pham.html` → `/product`, bỏ đuôi `.html` cho danh mục/thương hiệu/sản phẩm chi tiết).
- **Truy Cập Nhanh lên đầu:** đổi thứ tự trong `dashboard.html`, chuyển khối lưới link nhanh lên trước số liệu thống kê và tổng quan danh mục.
- **URL sản phẩm:** user muốn `/san-pham.html` hiện thành `/product`. Đã kiểm tra thật bằng curl vào domain production trước khi đổi: GitHub Pages tự phục vụ URL không đuôi (`/san-pham` → nội dung `san-pham.html`, không redirect, giữ nguyên URL) - xác nhận khả thi trước khi làm, không phải đoán.
- **2 mức độ, user chọn mức 1 (nhanh, an toàn):** (1) đổi hẳn tên file `san-pham.html` → `product.html`, phục vụ tại `/product`; (2) chỉ bỏ đuôi `.html` khi tạo link tới `category-chi-tiet`/`brand-san-pham`/`san-pham-chi-tiet` (giữ nguyên `?id=...`, KHÔNG đổi thành path `/product/man-hinh-o-to` - mức đó cần tạo file tĩnh riêng cho từng danh mục, để dành nếu sau này cần).
- **Rủi ro cần tránh:** `duchieuauto-backend/admin/` có sẵn 1 file **khác** cũng tên `san-pham.html` (trang quản lý sản phẩm CMS) - hoàn toàn không liên quan tới trang công khai. Rà kỹ từng chỗ tham chiếu trước khi thay hàng loạt, chỉ sửa 17 file công khai (12 trang tĩnh + 3 file JS) + đúng 2 chỗ trong admin thật sự trỏ RA trang công khai (`admin/banner.html` - placeholder gợi ý link banner, `admin/danh-gia.html` - link "xem sản phẩm" mở tab mới sang trang công khai), không đụng bất kỳ chỗ nào admin tự trỏ tới trang quản lý sản phẩm của chính nó.
- **Không để hỏng link/bookmark cũ:** giữ lại 1 file `san-pham.html` mới (thay cho file gốc đã đổi tên) chỉ làm nhiệm vụ chuyển hướng JS (`location.replace`) sang `/product`, giữ nguyên query string nếu có - ai lỡ có link/bookmark cũ vẫn tới đúng chỗ, không 404.
- **Dọn dẹp liên quan để đồng bộ:** cập nhật `scripts/generate-sitemap.js` (trang chủ ghi thẳng gốc domain thay vì `index.html`, `san-pham.html`→`product`, bỏ `.html` ở danh mục/thương hiệu/sản phẩm) rồi chạy lại sinh `sitemap.xml` mới (335 URL); cập nhật 4 dòng `posts.cta_link` trong Turso còn lưu định dạng `category-chi-tiet.html?id=...` cũ.
- **Test:** nâng cấp server tĩnh test local (`static-server.js` ở scratchpad) để giả lập đúng hành vi "URL không đuôi" của GitHub Pages (trước đó chỉ trả đúng 1-1 theo tên file, không mô phỏng được hành vi này) - nếu không nâng cấp sẽ không test được cục bộ trước khi đẩy lên. Xác nhận qua curl (`/product`, `/category-chi-tiet?id=...`, `/brand-san-pham?id=...`, `/san-pham-chi-tiet?id=...` đều 200) + Chrome headless (không lỗi console, tiêu đề trang đúng, link render ra đúng định dạng mới, trang chuyển hướng `san-pham.html` cũ chuyển đúng sang `/product`).
- **Tăng Service Worker cache lên v7.**

**2026-07-30 (lần 14)** — Thêm ảnh minh hoạ cho "nhóm sản phẩm" theo thương hiệu (VD "Loa Sub" của JBL trong Âm Thanh - Cách Âm Ô Tô).
- **Phát hiện:** cột `brand_types.logo` đã có sẵn trong DB từ trước, và `catalog-render.js` (`renderBrandProducts()`) đã đọc `t.logo` để render `<div class="brand-logo"><img>...` ngay trong thẻ chọn nhóm - phần hiển thị công khai đã sẵn sàng 100%, chỉ thiếu đúng 1 chỗ: không có cách nào (API lẫn giao diện admin) để GÁN logo cho 1 nhóm - `POST /admin/brand-types` (tạo nhóm mới) luôn ghi `logo = NULL`, và trước giờ chưa từng có route PUT nào cho `brand_types`.
- **Backend:** thêm `logo` vào `SELECT`/response của `GET /api/products/admin/categories` (trước đây chỉ trả `{id, name}` cho mỗi nhóm); thêm mới `PUT /api/products/admin/brand-types/:category_id/:brand_id/:id` (nhận `name`/`logo`, sửa được sau khi tạo - trước đây không có PUT/DELETE nào cho `brand_types`, chỉ có POST tạo).
- **Admin UI:** thêm khối "🖼️ Ảnh Minh Hoạ Nhóm Sản Phẩm" vào `danh-muc.html` (trang đã có sẵn dropdown chọn danh mục) - liệt kê từng thương hiệu CÓ nhóm con trong danh mục đang chọn, mỗi nhóm 1 ô tải ảnh (dùng lại đúng cơ chế upload đã có ở `banner.html`/form sản phẩm: chọn file → `POST /api/uploads` purpose `square` → PUT lưu URL luôn, không cần bấm Lưu riêng).
- **Test:** dùng đúng ví dụ user nêu (JBL/Loa Sub, danh mục Âm Thanh - Cách Âm Ô Tô) - PUT/GET qua curl xác nhận round-trip đúng, dump DOM xác nhận cả trang admin lẫn trang công khai `brand-san-pham?...&brand=jbl` đều hiện đúng ảnh test, sau đó revert `logo` về `null` và xoá tài khoản test - không để lại dữ liệu test nào trong Turso production. Không cần tăng version Service Worker (chỉ sửa backend + trang admin, không đụng file tĩnh công khai nào).
- **[MỚI] Bổ sung xong 3 sản phẩm còn thiếu ảnh gốc hoàn toàn** (kovar-t2, safeview-elite-360, finevu-gx4 - phát hiện lúc làm cảnh báo ở trên) - user yêu cầu tự tìm trên trang hãng/nhà phân phối. Đã search + WebFetch lấy ảnh từ auto365.vn (Kovar T2), akauto.com.vn (Safeview Elite 360 - phải dùng sharp cắt bỏ dải logo watermark "AKauto" của nhà bán lẻ khác trước khi dùng), garageaudio.com.au (FineVu GX4K - phát hiện tên đúng là "GX4K" không phải "GX4", đã sửa lại tên sản phẩm) - đối chiếu ảnh bằng mắt xác nhận đúng sản phẩm trước khi lưu. Thêm luôn bảng thông số kỹ thuật thật (product_specs) lấy từ cùng nguồn. Chạy `generate-responsive-images.js` tạo bản "-400w", xác nhận cảnh báo "thiếu ảnh" ở Dashboard đã về 0 danh mục.
- **User duyệt demo, yêu cầu Claude tự tìm và điền ảnh cho TẤT CẢ nhóm** thay vì để user tự tải từng cái. Rà toàn bộ `brand_types`: 36 nhóm, trong đó 8 nhóm ở "Đồ Bán Tải" đã có sẵn logo thương hiệu thật (King Springs/TJM/Aeroklas/WARN, gán từ trước) - **giữ nguyên, không đụng**; 30 nhóm còn trống (Âm Thanh: Loa Ô Tô/Loa Sub/Âm Ly × 4 thương hiệu; Dán Phim: theo số chỗ ngồi; Độ Đèn: Bi LED/Bi Gầm × 4 thương hiệu; Màn Hình: Có/Không Camera 360 × 3 thương hiệu). Với mỗi nhóm trống, lấy ảnh sản phẩm THẬT đầu tiên (còn hiển thị, đúng thương hiệu + đúng nhóm đó) làm ảnh minh hoạ - tránh phải tìm ảnh ngoài (không có công cụ tìm ảnh, và ảnh sản phẩm thật của đúng thương hiệu/nhóm chính xác hơn ảnh minh hoạ chung chung). Đã kiểm tra cả 30 ảnh chọn ra đều tồn tại thật trên đĩa trước khi ghi vào DB (không lặp lại lớp lỗi "ảnh vỡ" của Phase 11). Áp dụng thẳng bằng script, xác nhận lại qua API production thật - **đây là thay đổi DỮ LIỆU (Turso), có hiệu lực ngay lập tức, không cần git push/deploy.**

**2026-08-03 (lần 15)** — Đổi email liên hệ hiển thị trên web từ gmail cá nhân sang `info@duchieuauto.vn` (mail ảo, tự chuyển tiếp).
- **User muốn** web hiện `info@duchieuauto.vn` thay vì lộ thẳng `duchieuauto47@gmail.com`, nhưng mail gửi tới `info@` vẫn phải vào đúng gmail đó. Domain `duchieuauto.vn` lúc kiểm tra chưa có MX record nào (chưa từng cấu hình mail) - làm mới hoàn toàn, không xung đột.
- **Hạ tầng (Cloudflare Email Routing, user tự thao tác, Claude hướng dẫn từng bước qua ảnh chụp màn hình - đúng cách đã làm lúc chuyển DNS domain trước đây):** Email Routing → chọn zone `duchieuauto.vn` → Destination Addresses thêm + xác nhận `duchieuauto47@gmail.com` → Routing rules tạo rule `info@duchieuauto.vn` → gửi tới `duchieuauto47@gmail.com` (Active). Cloudflare tự thêm 3 MX record cần thiết.
- **Thứ tự quan trọng:** Claude chủ động yêu cầu hoàn thành + xác nhận rule Active ở Cloudflare TRƯỚC khi đổi hiển thị trên web - nếu đổi web trước mà mail chưa route được thật, khách bấm gửi mail sẽ bị dội, mất liên hệ.
- **Code (sau khi hạ tầng xong):** thay `duchieuauto47@gmail.com` → `info@duchieuauto.vn` ở 42 chỗ trên 13 trang tĩnh (đầu trang, footer, JSON-LD, trang chính sách...) bằng script thay hàng loạt (không đụng file nào trong `duchieuauto-backend/` vì không có tham chiếu email này ở backend). Tăng Service Worker cache lên v8.

**2026-08-03 (lần 16)** — Rà soát toàn site trên giao diện di động (yêu cầu user "mở tab điện thoại, kiểm tra mọi thứ"): phát hiện + sửa 1 lỗi Service Worker làm vỡ ảnh ngẫu nhiên, 1 lỗi CSS làm hiện sai thanh CTA dính đáy, nhiều chỗ chữ bị đè bởi widget liên hệ nhanh, và 1 lỗi cấu hình reCAPTCHA nghiêm trọng làm 3 form công khai không gửi được.
- **Công cụ mới:** `mobile-shot.js` (script Node dùng CDP WebSocket trực tiếp, không qua thư viện ngoài) - mở Chrome headless với `Emulation.setDeviceMetricsOverride({mobile:true})` (giả lập di động ĐÚNG, khác hẳn cờ CLI `--window-size` từng gây 1 lần báo nhầm "chữ Hero bị cắt" ở phiên trước - đã kiểm chứng lại bằng CDP thật và xác nhận đó là báo nhầm), tự ép hiện các phần tử AOS, cuộn tới nhiều vị trí rồi chờ ảnh trong khung nhìn tải xong hẳn trước khi chụp - tránh chụp nhầm lúc ảnh đang tải dở thành "ảnh vỡ" giả.
- **Lỗi thật 1 - Service Worker (`sw.js`) làm vỡ ảnh hợp lệ khi nhiều ảnh khác URL tải gần như đồng thời:** phát hiện ảnh trong khối Giới Thiệu (`about_image_detail1/2`) và dải logo thương hiệu cuộn ngang bị vỡ ngẫu nhiên dù file và mạng đều không lỗi - `Network.responseReceived` xác nhận Service Worker vẫn trả `status:200` nhưng nội dung ảnh trống (`naturalWidth:0`). Đã thử 2 hướng vá cache-logic (cô lập lỗi `cache.put()`, rồi bọc try/catch cho cả `cache.match()`/`caches.open()`) nhưng **không đủ** - kiểm chứng bằng cách tắt hẳn Cache Storage (passthrough `fetch()` thuần) vẫn tái hiện y hệt, kể cả trên production (`duchieuauto.vn`) khi so sánh có/không `Network.setBypassServiceWorker`. **Kết luận: bản thân cơ chế `event.respondWith(fetch(req))` của Service Worker không ổn định khi bị dội nhiều request ẢNH khác URL gần như đồng thời** (VD dải logo lazy-load nhiều ảnh cùng lúc khi cuộn tới). **Fix cuối cùng:** loại hẳn request ẢNH ra khỏi phạm vi can thiệp của Service Worker (thêm điều kiện bỏ qua theo đuôi file ảnh, y hệt cách đã loại trừ `/api/`) - ảnh vốn đã có HTTP cache riêng của trình duyệt nên không mất gì, chỉ còn CSS/JS/HTML đi qua Service Worker. Xác nhận lại bằng ảnh trước/sau: cả ảnh Giới Thiệu lẫn toàn bộ logo thương hiệu tải đúng. Đồng thời phát hiện riêng 1 lỗi thứ 2 góp phần gây ra triệu chứng ban đầu: `homepage-content.js` gán lại `el.src` KHÔNG ĐIỀU KIỆN dù giá trị từ DB giống hệt `src` gốc trong HTML, khiến trình duyệt tải trùng request cho cùng 1 ảnh - đã thêm điều kiện chỉ gán khi giá trị thật sự khác. Tăng Service Worker cache lên v9.
- **Lỗi thật 2 - `.sticky-product-cta` (thanh CTA dính đáy ở trang chi tiết sản phẩm) LUÔN HIỆN dù JS đã tắt đúng:** JS (`setupStickyProductCta` trong `catalog-render.js`) gắn đúng thuộc tính `hidden` khi nút CTA gốc còn trong khung nhìn, nhưng CSS `.sticky-product-cta{display:flex;...}` cùng độ ưu tiên (specificity) với quy tắc mặc định `[hidden]{display:none}` của trình duyệt lại thắng (CSS tác giả luôn nằm sau stylesheet mặc định trong cascade) - kết quả: khách thấy 2 nút "Đặt Lịch Ngay" chồng nhau ngay từ đầu trang, mọi lúc, trên mọi trang sản phẩm. Vá bằng 1 dòng `.sticky-product-cta[hidden]{display:none}` để giành lại độ ưu tiên đúng.
- **Lỗi thật 3 - chữ bị widget "Liên hệ nhanh" (`.quick-contact`, cố định góc phải ~56px) đè lên, đọc mất chữ khi cuộn tới đúng khoảng widget đứng:** xảy ra ở mọi khối văn bản dài chạy gần sát mép phải - nội dung SEO danh mục (`category-seo-content`), mô tả sản phẩm (`product-detail-desc`), nội dung Chính Sách (`policy-content`), nội dung bài viết (`blog-detail-body`). Vá bằng `padding-right` (60px) riêng cho 4 khối này trong media query di động - còn vài chỗ chữ CĂN GIỮA (footer, mô tả ngắn) bị đè tuỳ nội dung/vị trí cuộn, mức độ nhẹ hơn (không mất nghĩa câu), ghi nhận làm khuyến nghị thay vì vá từng chỗ lẻ.
- **Lỗi thật 4 - khối thống kê "240+/30+/8" (Giới Thiệu) thừa 1 vạch dọc trơ trọi khi mục thứ 3 tự xuống dòng riêng ở màn hẹp:** `border-left` gắn theo kiểu "cạnh trái phần tử liền kề" (`.stat-item + .stat-item`) không biết phần tử đã xuống dòng khác - vá bằng xếp dọc hẳn 1 cột + đổi sang `border-top` ở `@media (max-width:576px)`.
- **Phát hiện nghiêm trọng, đã sửa - lỗi cấu hình reCAPTCHA từng chặn đứng 3 form công khai:** cả 3 form dùng chung 1 site key (`6LfmGmgtAAAAAAnu3WN7Oploh0ElwHSPR4X8cdQx`) - Đặt Lịch Hẹn, Nhắc Bảo Dưỡng, Gửi Đánh Giá Sản Phẩm - đều hiện lỗi **"ERROR for site owner: Invalid site key"** ngay trong khung reCAPTCHA, xác nhận lại trên chính production (`duchieuauto.vn`, không phải do localhost). Vì ô "Tôi không phải người máy" không bao giờ hiện ra bấm được, `grecaptcha.getResponse()` luôn rỗng, và validate phía client chặn gửi form ngay tại đó - **nhiều khả năng mọi lượt đặt lịch/đăng ký nhắc bảo dưỡng/gửi đánh giá qua các form này đều đang thất bại** (không phải lỗi giao diện di động, lỗi trên cả desktop, nhưng phát hiện được lúc rà theo yêu cầu audit di động).
  - User kiểm tra Google reCAPTCHA Admin Console: site key/domain/loại key (v2 Checkbox) đều hiển thị đúng, nhưng trang quản trị hiện dòng gợi ý "migrate SiteVerify sang CreateAssessment" - dấu hiệu key có thể đã bị gắn kiểu Enterprise trong khi code tích hợp theo kiểu cổ điển (`recaptcha/api.js` + `siteverify`), gây lệch. Thay vì mất công gỡ rối kiểu Enterprise, **user tạo mới 1 site key thuần cổ điển** tại `google.com/recaptcha/admin/create` (v2 Checkbox, domain `duchieuauto.vn` + `www.duchieuauto.vn`).
  - Đã thay `data-sitekey` cũ bằng key mới (`6Len0XItAAAAABrKtuCE9x7WuELoQUNl6qkV0VA1`) ở cả 3 file (`dat-lich-hen.html`, `nhac-bao-duong.html`, `san-pham-chi-tiet.html`). Xác nhận key mới hợp lệ bằng 2 cách: (1) gọi thẳng endpoint `recaptcha/api2/anchor` của Google bằng key mới - không còn thấy chữ lỗi; (2) mở trang thật qua Chrome headless, lỗi đổi từ "Invalid site key" sang "Localhost is not in the list of supported domains" - đúng như kỳ vọng (localhost vốn không nằm trong danh sách domain đã đăng ký, chỉ `duchieuauto.vn` mới có, tức bản thân key giờ đã hợp lệ).
  - **Secret Key mới (`6Len0XItAAAAABwXMhpmuaHydpiAUVplXGYlOhly`) KHÔNG đưa vào code/git** - cần user tự cập nhật biến môi trường `RECAPTCHA_SECRET_KEY` trên Render (`utils/verify-recaptcha.js` đọc từ `process.env`, đã có sẵn cơ chế bỏ qua xác thực an toàn nếu biến trống nên form vẫn hoạt động được kể cả trước khi kịp cập nhật biến này, chỉ là chưa xác thực thật ở backend).
- **Chưa push:** đang chờ user xem demo local + đọc báo cáo phát hiện trước khi đẩy lên, theo đúng quy trình đã thống nhất.
- **Test:** Chrome headless xác nhận trang chủ không còn sót `duchieuauto47@gmail.com` nào, không lỗi console, trước khi đưa demo cho user duyệt.

**2026-08-03 (lần 17)** — Loạt yêu cầu chỉnh giao diện + dữ liệu trang chủ sau khi xem demo di động thật trên điện thoại (ảnh chụp màn hình thật, không phải giả lập).
- **Widget "Liên hệ nhanh" (`.quick-contact`) mặc định thu gọn trên di động:** trước đó luôn mở sẵn 3 nút cao ~200px che khá nhiều nội dung khi cuộn. Thêm 1 dòng kiểm tra `matchMedia('(max-width:768px)')` ngay sau khi lấy phần tử, y hệt trên cả 13 trang tĩnh (nút bấm mở/đóng vẫn hoạt động bình thường, chỉ đổi trạng thái MẶC ĐỊNH lúc tải trang).
- **Card blog 2 cột (`assets/js/blog-render.js`, `pages.css`):** tách "danh mục" và "ngày đăng" thành 2 dòng riêng (`.blog-card-cat` cắt gọn 1 dòng bằng ellipsis nếu quá dài, `.blog-card-date` dòng riêng) thay vì nhét chung 1 dòng rồi wrap lộn xộn tuỳ độ dài tên danh mục.
- **2 ô "Đặt Lịch Hẹn"/"Nhắc Bảo Dưỡng" trên trang chủ:** theo đúng ảnh chụp thật user gửi (mỗi ô cao gần hết màn hình, phải cuộn nhiều) - đổi `.home-cta-grid` giữ NGUYÊN 2 cột cạnh nhau ở di động (bỏ rule cũ auto xếp chồng 1 cột), thu nhỏ hẳn font/icon/khoảng đệm cho vừa cột hẹp.
- **Số liệu thống kê ("240+/30+/8") thay bằng số liệu THẬT:** tạo tài khoản admin tạm (`audit_temp`, xoá ngay sau khi dùng xong đúng quy trình) gọi `GET /admin/categories/overview` (endpoint Dashboard đã có sẵn) đếm trực tiếp từ Turso - kết quả thật: **255 sản phẩm, 35 thương hiệu (đếm theo tên duy nhất qua toàn bộ danh mục), 11 danh mục** (số "8" cũ đã lỗi thời - có thêm nhiều danh mục dịch vụ mới từ Phase 9). Cập nhật qua đúng API CMS `PUT /api/homepage-content` (không sửa thẳng DB) thành `250+/35+/11` (làm tròn xuống một chút để không phải sửa lại ngay khi kho hàng dao động nhẹ), đồng thời cập nhật `seed-homepage-content.js` (script seed 1 lần) để khớp nếu có ai chạy lại trên môi trường mới.
- **Đồng bộ kích thước 3 khối thẻ trang chủ (Dịch Vụ / Sản Phẩm Chiến Lược / Giải Mã Công Nghệ) theo đúng yêu cầu lấy "Giải Mã Công Nghệ" làm mẫu:** so sánh CSS thấy 3 khối đang lệch nhau khá nhiều (border-radius 8px/14px khác nhau, ảnh 175-210px khác nhau, có khối bị thu nhỏ riêng ở di động - có khối thì không). Áp `border-radius:14px`, ảnh cao 200px, hover nhấc 8px, padding/font tiêu đề-mô tả giống hệt `.tech-story-card`:
  - `.product-card` (Sản Phẩm Chiến Lược): sửa thẳng vì chỉ dùng riêng ở trang chủ, không dùng chung chỗ nào khác.
  - `.service-card` (Dịch Vụ): class này DÙNG CHUNG với lưới danh mục ở trang `/product` (2 cột, cần giữ nhỏ gọn) - viết riêng bộ quy tắc scope theo `.service-slider .service-card` (độ ưu tiên 2-class) để CHỈ đổi trong phạm vi slider trang chủ, không đụng gì tới trang `/product`. Đã dump DOM xác nhận lưới `/product` không đổi gì sau khi sửa.
  - Vị trí mũi tên điều hướng (swiper) tính lại theo `top = nửa chiều cao ảnh mới` (200px → 100px), viết dạng scope riêng theo từng slider (đúng pattern tech-slider/service-slider đã có sẵn) thay vì sửa rule mặc định dùng chung toàn site - tránh ảnh hưởng slider khác không liên quan.
- **Địa chỉ cửa hàng đổi số nhà 123 → 251 (`251 Lý Thái Tổ, Phường Buôn Ma Thuột, Đắk Lắk`):** rà toàn bộ 13 trang tĩnh (29 vị trí) + JSON-LD (`streetAddress`) + admin placeholder + seed script bằng script thay hàng loạt. **Phát hiện phụ lúc rà:** 2 link Google Maps ("Ghé Cửa Hàng" + iframe bản đồ nhúng) trong `index.html` từ trước tới giờ đã bị sai dấu trong chuỗi URL-encode (`Buôn Ma Thuật`/`Đăk Lăk` thay vì `Buôn Ma Thuột`/`Đắk Lắk`) - không phải lỗi do lần sửa này, có sẵn từ trước, tiện thể sửa luôn đúng dấu + số nhà mới. Xác nhận `info@duchieuauto.vn` đã hiển thị đúng toàn site từ lần 15, không cần sửa gì thêm.
- **Test:** mỗi phần đều dump DOM/screenshot qua Chrome headless (CDP) trước/sau khi sửa để xác nhận trực quan, không chỉ đọc code. Toàn bộ 20 file thay đổi được xem lại qua `git diff --stat` trước khi user duyệt push.
