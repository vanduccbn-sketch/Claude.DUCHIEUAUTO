...
router.post("/", canEditPosts, async (req, res) => {
    const { title, category, excerpt, cover_image, content, meta_title, meta_description, cta_text, cta_link, published } = req.body;
    if (!title || !content) {
    ...
    const info = await db.prepare(`
        ...
        VALUES (@slug, @title, @category, @excerpt, @cover_image, @content, @meta_title, @meta_description, @cta_text, @cta_link, @published)
    `).run({
        slug, title,
