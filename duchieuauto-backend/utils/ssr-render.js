/* =========================================================
   SSR cho bot (dynamic rendering - Phase 12, 2026-08-05)
   Sinh HTML tĩnh có đủ title/meta/OG/schema.org + nội dung thật (tên, giá, mô tả, thông số, FAQ)
   cho sản phẩm/danh mục/bài viết - dùng riêng cho bot không chạy JavaScript (phần lớn AI crawler:
   GPTBot, ClaudeBot, PerplexityBot... và bot mạng xã hội: Facebook, Zalo, Telegram...).

   Không phải SSR cho người dùng thật - trang công khai vẫn là site tĩnh + JS như cũ, không đổi gì.
   Cloudflare Worker (xem duchieuauto-backend/cloudflare-worker-bot-ssr.js) sẽ nhận diện đúng
   User-Agent bot rồi gọi các route /render/* này thay vì để bot thấy khung trang rỗng.

   Nguyên tắc: nội dung SSR phải TƯƠNG ĐƯƠNG (không nhiều/ít hơn) nội dung JS thực sự render ra cho
   người dùng thật - đúng khuyến nghị "dynamic rendering" của Google, không phải cloaking. Toàn bộ
   field đọc thẳng từ cùng 1 database, cùng công thức title/meta/schema như assets/js/catalog-render.js
   và assets/js/blog-render.js phía client.
   ========================================================= */
...
/* ---------- Danh mục ---------- */
function renderCategoryHtml(cat, sections, faqs) {
